<template>
  <view class="page">
    ...
    <view class="section">
      ...
      <view v-for="stat in monthlyStats" :key="`${stat.year}-${stat.month}`" class="month-card">
    ...
    <view class="section">
      ...
      <view class="chart-box">
        <view v-for="stat in monthlyStats" :key="`bar-${stat.year}-${stat.month}`" class="bar-col">
...
<script setup lang="ts">
  ...
  const totalWorkDays = computed(() => monthlyStats.value.reduce((s, v) => s + v.totalWorkDays, 0))
  const totalActualDays = computed(() =>
    monthlyStats.value.reduce((s, v) => s + v.actualWorkDays, 0),
  )
  const totalLeaveDays = computed(() => monthlyStats.value.reduce((s, v) => s + v.leaveDays, 0))
