  ...
  <view class="page">
      ...
      <view class="day-grid">
        <view
          ...
          :class="{
            ...
            selected:
              selectedDay && cell.day && cell.fullDate === selectedDay.fullDate,
          }"
    ...
    <view class="detail-card" v-if="selectedDay">
      ...
      <view v-else class="detail-form">
        ...
        <view
          class="form-row hours-row"
          v-if="editClockIn && editClockOut && !editIsLeave"
        >
          <text class="form-label">工时</text>
          <text class="hours-value"
            >{{ formatHours(editClockIn, editClockOut) }}h</text
          >
        </view>
...
const calendarCells = computed<CalendarCell[]>(() => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cells.push({
      ...
      isOvertime: (isHol || (isWkend && !isWorkdayFromLib(dateStr))) && !!record && !record.isLeave && !!record.clockIn,
